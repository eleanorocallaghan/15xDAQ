...
for i = 1:TotalTime/2
    if RawData(i,he) > 20 && RawData(i+3*SamplingFrequency,he) > 20 %records last pt for which HE=0 before car starts
        firstCutoff = i;
        break;
    end
end
for j = TotalTime:-1:TotalTime/2
    if RawData(j,he) > 10 && RawData(j-1*SamplingFrequency,he) > 10%records last pt where HE=0 after car stops
        finalCutoff = j;
        break;
    end
end
